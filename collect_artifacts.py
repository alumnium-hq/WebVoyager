...
PROJECT_ROOT = Path(__file__).parent.parent.parent  # alumnium root
                        ...
                        for block in msg_content:
                            if (
                                ...
                                if tid and tid in pending_tool_calls and result_text:
                                    pending_tool_calls[tid]["tool_response"] = (
                                        result_text
                                    )
    ...
    if artifacts_dir:
        ...
        if screenshots_dir.exists():
            ...
            for i, src in enumerate(sorted(screenshots_dir.glob("*.png")), 1):
                shutil.copy(src, output_dir / f"screenshot{i}.png")
        ...
        if token_stats_path.exists():
            ...
            print(f"Warning: token-stats.json not found in {artifacts_dir}")
            token_stats_path = None
...
def load_task_from_dataset(task_id: str) -> dict | None:
    """Load task question and URL from WebVoyager dataset by ID."""
    data_file = Path(__file__).parent / "data" / "WebVoyager_data.jsonl"
    try:
...
def recollect_artifacts(task_dir: Path) -> bool:
    ...
    Uses existing transcript.jsonl to regenerate interact_messages.json,
    metadata.json, and copy Alumnium artifacts.
    Returns True if transcript was found and processed.
    """
    ...
    # Derive task_id from directory name (strip leading "task")
    task_id = task_dir.name.removeprefix("task")
    task = load_task_from_dataset(task_id)
    if task:
        question = task["ques"]
        url = task["web"]
        print(f"  question: {question[:60]}...")
    else:
        # Fall back to existing metadata if available
        metadata_path = task_dir / "metadata.json"
        if metadata_path.exists():
            try:
                existing = json.loads(metadata_path.read_text())
                question = existing.get("question", "")
                url = existing.get("start_url", "")
            except Exception:
                question, url = "", ""
        else:
            question, url = "", ""
        print("  Warning: task not found in dataset, using question/url from metadata")

    # Regenerate interact_messages.json
    final_answer = extract_final_answer(str(transcript_path))
    interact_messages = convert_to_interact_messages(
        str(transcript_path), question, url, final_answer
    )
    (task_dir / "interact_messages.json").write_text(
        json.dumps(interact_messages, indent=2)
    )
    print(f"  Generated interact_messages.json ({len(interact_messages)} messages)")

    # Try to find and copy Alumnium artifacts
    ...
    if artifacts_dir:
        ...
        if screenshots_dir.exists():
            ...
            for i, src in enumerate(sorted(screenshots_dir.glob("*.jpg")), 1):
                ...
                count += 1
            for i, src in enumerate(sorted(screenshots_dir.glob("*.png")), 1):
                shutil.copy(src, task_dir / f"screenshot{i}.png")
                count += 1
        ...
        if token_stats_path.exists():
            ...
            print("  Copied token-stats.json")
        else:
            token_stats_path = None
    else:
        print(f"  Artifacts directory not found (driver_id: {driver_id})")

    # Regenerate or update metadata.json
    costs = calculate_costs(str(transcript_path), token_stats_path)
    metadata_path = task_dir / "metadata.json"
    if metadata_path.exists():
        try:
