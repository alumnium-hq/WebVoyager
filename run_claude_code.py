...
SCRIPT_DIR = Path(__file__).parent  # benchmarks/webvoyager
DATA_FILE = SCRIPT_DIR / "data" / "WebVoyager_data.jsonl"
RESULTS_DIR = SCRIPT_DIR / "results" / "claude-code"

VIEWPORT_SCREENSHOT_TASKS = {"Coursera--39"}
...
def format_message(data: dict, shown_content: dict) -> None:
    """Format and print a JSON message from Claude Code stream-json output."""
    ...
    if msg_type == "assistant":
        ...
        already_shown = shown_content.get(msg_id, 0)
        new_items = content[already_shown:]
        shown_content[msg_id] = len(content)

        for item in new_items:
            item_type = item.get("type")
            if item_type == "thinking":
                text = (item.get("thinking") or "").replace("\n", " ")
                if text:
                    print(f"\033[90m[thinking] {text}\033[0m", flush=True)
            elif item_type == "tool_use":
                name = item.get("name", "")
                inp = json.dumps(item.get("input", {}))
                print(f"\033[34m[tool] {name}({inp})\033[0m", flush=True)
            elif item_type == "text":
                text = item.get("text", "")
                if text:
    ...
    if msg_type == "user":
        for item in data.get("message", {}).get("content", []):
            ...
            if item.get("is_error"):
                print(f"\033[31m[error] {result_str}\033[0m", flush=True)
            else:
                print(f"\033[90m[result] {result_str}\033[0m", flush=True)
...
def run_task(task_id: str, timeout: int = 1200) -> dict | None:
    ...
    env.update(
        {
            ...
            "ALUMNIUM_EXCLUDE_ATTRIBUTES": "url",  # Exclude url attribute to avoid context limits on Amazon
            "ALUMNIUM_FULL_PAGE_SCREENSHOT": "true",
            "ALUMNIUM_MODEL": "azure_openai/gpt-5-nano",
            "ALUMNIUM_PLANNER": "false",
        }
    )

    # Use undetected chromedriver for sites that block automation detection
    if task_id in DISGUISED_TASKS or task.get("web_name") in DISGUISED_GROUPS:
        env["ALUMNIUM_USE_UNDETECTED_CHROMEDRIVER"] = "true"

    # Full-page screenshots cause UI to reset on some sites like Coursera
    if task_id in VIEWPORT_SCREENSHOT_TASKS:
        env["ALUMNIUM_FULL_PAGE_SCREENSHOT"] = "false"
    ...
    process = subprocess.Popen(
        ...
        env=env,
        cwd=str(SCRIPT_DIR),
        stdin=subprocess.DEVNULL,  # Prevent subprocesses from accessing/modifying terminal
        stdout=subprocess.PIPE,
        text=True,
        bufsize=1,
        start_new_session=True,  # Isolate from our process group so Ctrl-C doesn't propagate
    )
    assert process.stdout is not None
    try:
        while True:
            if time.time() - start_time > timeout:
                kill_process_group(signal.SIGINT)
                timed_out = True
                break
            ...
            if ready:
                ...
                if line:
                    try:
                        format_message(json.loads(line), shown_content)
